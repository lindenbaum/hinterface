...
spec = do
  describe "Name" $ do
    ...
    it "encodes as expected" $
      encode (Name R6B (DistributionFlags []) "name")
        `shouldBe` withLength16
          ( "n" `LBS.append` LBS.pack [0, 5, 0, 0, 0, 0]
              `LBS.append` "name"
          )
  ...
  describe "Status" $ do
    ...
    it "OkSimlutaneous encodes to \"ok_simultaneous\"" $
      encode OkSimultaneous
        `shouldBe` withLength16 ("s" `LBS.append` "ok_simultaneous")
    it "Nok encodes to \"nok\"" $
      encode Nok `shouldBe` withLength16 ("s" `LBS.append` "nok")
    it "NotAllowed encodes to \"not_allowed\"" $
      encode NotAllowed
        `shouldBe` withLength16 ("s" `LBS.append` "not_allowed")
...
withLength16 bytes =
  encode (fromIntegral (LBS.length bytes) :: Word16)
    `LBS.append` bytes
