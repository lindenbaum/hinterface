...
spec = do
    describe "toIOx" $ do
        it "converts IO errors" $
            (runIOx $ (toIOx $ ioError (userError "ioError")) `catchX` (return . show)) `shouldBe`
                "user error (ioError)"
    describe "throwX" $ do
        it "throws an error" $
    ...
    describe "forever" $ do
        it "may throw an error immediately" $
            (runIOx $ forever actionX `catchX` handler2) `shouldBe` 2
        it "may throw an error after some iterations" $ do
            (runIOx $ do
                 counter <- liftIO $ newIORef (0 :: Int)
                 (forever $ do
                      val' <- liftIO $ atomicModifyIORef' counter (\val -> (val + 1, val + 1))
                      when (val' >= 42) $
