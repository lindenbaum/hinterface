    ...
    put controlMessage = putWithLength32be $ do
      ...
      where
        ...
        put' (LINK fromPid toPid) =
            put (ExternalTerm (linkTag, fromPid, toPid))

        put' (SEND toPid message) = do
            put (ExternalTerm (sendTag, unused, toPid))
            put (ExternalTerm message)

        put' (EXIT fromPid toPid reason) =
            put (ExternalTerm (exitTag, fromPid, toPid, reason))

        put' (UNLINK fromPid toPid) =
            put (ExternalTerm (unlinkTag, fromPid, toPid))

        put' NODE_LINK =
            put (ExternalTerm (Tuple1 nodeLinkTag))

        put' (REG_SEND fromPid toName message) = do
            put (ExternalTerm (regSendTag, fromPid, unused, toName))
            put (ExternalTerm message)

        put' (GROUP_LEADER fromPid toPid) =
            put (ExternalTerm (groupLeaderTag, fromPid, toPid))

        put' (EXIT2 fromPid toPid reason) =
            put (ExternalTerm (exit2Tag, fromPid, toPid, reason))
    get = do
        ...
        if expectedLen == 0
            then return TICK
                ...
                if fromIntegral expectedLen == actualLen
                    then pure controlMessage
                    else fail ("Bad control message length: " ++ show actualLen
                              ++ " expected lenght: " ++ show expectedLen
                              ++ " controllMessage content: " ++ show controlMessage)
      where
        badControlMsg term = fail ("Bad control message: " ++ show term)
        get' = do
            ExternalTerm term <- get
