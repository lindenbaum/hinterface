...
data TReference =
          TReference TAtom !Word32 !Word8
        | TNewReference TAtom !Word8 ![Word32]
        | TNewerReference
data TPort =
    TPort TAtom !Word32 !Word8
    | TNewPort
data TPid =
    TPid !TAtom !Word32 !Word32 !Word8
    | TNewPid
...
data GTerm t where
    ...
    GTuple1 :: !(TTuple '[a1]) -> GTerm (TTuple1 a1)
    GTuple2 :: !(TTuple '[a1, a2])-> GTerm (a1, a2)
    GTuple3 :: !(TTuple '[a1, a2, a3])-> GTerm (a1, a2, a3)
    GTuple  :: !(TTuple (a1 ': a2 ': a3 ': as)) -> GTerm (TTuple (a1 ': a2 ': a3 ': as))
    GNil :: GTerm ()
    GList :: ![GTerm a] -> GTerm [a]

data TTuple ts where
    TupleNil :: TTuple '[]
    (:|) :: !(GTerm t) -> !(TTuple ts) -> TTuple (t ': ts)
...
instance ToGTerm (TTuple (a1 ': a2 ': a3 ': as)) where toGTerm = GTuple
instance ToGTerm t => ToGTerm (TTuple1 t) where
    toGTerm (TTuple1 t) = GTuple1 (toGTerm t :| TupleNil)
instance (ToGTerm t1, ToGTerm t2) => ToGTerm (t1, t2) where
    toGTerm (t1, t2) = GTuple2 (toGTerm t1 :| toGTerm t2 :| TupleNil)
instance (ToGTerm t1, ToGTerm t2, ToGTerm t3) => ToGTerm (t1, t2, t3) where
    toGTerm (t1, t2, t3) = GTuple3 (toGTerm t1 :| toGTerm t2 :| toGTerm t3 :| TupleNil)
...
fromGTerm (GTuple1    (e1    :|TupleNil)) = TTuple1 (fromGTerm e1)
fromGTerm (GTuple2    (e1:|e2:|TupleNil)) = (fromGTerm e1, fromGTerm e2)
fromGTerm (GTuple3 (e1:|e2:|e3:|TupleNil)) =
